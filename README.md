...
13.1 웹소켓 소개
- 웹은 HTTP 프로토콜 위에서 동작하고 있기 때문에 요청을 보내야지만 서버가 응답을 주게 됩니다. 즉 양방향 통신을 지원하지 않습니다.
- 폴링: 주기적으로 요청을 보내는 방식
- 롱폴링: 클라이언트와 서버간의 커넥션을 유지한 상태로 응답을 주고 받는 방식
- 폴링이나 롱폴링 둘다 클라이언트가 서버에게 요청을 보내야 합니다.
- 웹소켓: 하나의 TCP 커넥션으로 서버와 클라이언트 간에 양방향 통신을 할수 있게 만든 프로토콜, 웹소켓은 양방향 통신을 지원하므로 브라우저 상에서 리프레시 없이 실시간성을 요구하는 애플리케이션을 구현할수 있습니다.
  - 양방향 통신: 데이터의 송수신을 동시에 처리한다는 뜻이며 클라이언트와 서버가 원하는 때 데이터를 주고받을수 있다는 의미, 통상적인 HTTP 통신은 클라이언트가 요청하는 때만 서버가 응답하는 단방향 통신
  - 실시간 네트워킹을 구현하는 것이 용이: 웹 환경에서 연속된 데이터를 빠르게 노출하고 싶을때, 채팅이나 주식앱에 적합, 또한 브로드캐스팅을 지원하므로 여러 클라이언트와 빠르게 데이터를 교환할수 있다.
